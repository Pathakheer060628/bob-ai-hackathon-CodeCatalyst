...
export default function LoadBalancePanel({ plan }) {
  ...
  if (!plan.feasible) {
    return (
      <div className="card">
        ...
        <p style={{ color: "var(--status-critical)", fontSize: 13 }}>No feasible plan found for this horizon.</p>
      </div>
    );
  }
  ...
  return (
    <div className="card">
      ...
      <h3 style={{ marginTop: 16 }}>Recommended actions{!showAll && rows.length ? ` (${rows.length} active hour${rows.length === 1 ? "" : "s"})` : ""}</h3>
      {rows.length === 0 ? (
        <p style={{ color: "var(--text-secondary)", fontSize: 13 }}>No storage or demand-response action needed this horizon.</p>
      ) : (
